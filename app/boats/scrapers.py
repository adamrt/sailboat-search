...
class YachtWorldItem(object):
    ...
    def process(self):
        ...
        if self.boat.length_from_name != self.length_from_name:
            print(
                "{} != {} :: {} || {}".format(
                    self.boat.length_from_name,
                    self.length_from_name,
                    self.boat.name,
                    self.name,
                )
            )
        ...
        listing, created = Listing.objects.get_or_create(
            url=self.url, defaults={"boat": self.boat, "title": self.name}
        )
    ...
    def name(self):
        return " ".join(
            self.soup.find("div", {"class": "make-model"}).find("a").text.split()
        )

    @property
    def price(self):
        s = (
            self.soup.select(".price")[0]
            .text.strip()
            .replace("US$", "")
            .replace("*", "")
            .replace(",", "")
